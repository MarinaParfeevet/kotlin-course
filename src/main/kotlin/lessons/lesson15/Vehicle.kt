...
//abstract класс — это базовый класс, из которого нельзя создавать объекты напрямую. Он содержит абстрактные методы
// (без реализации), которые должны быть реализованы в наследниках. Используется, когда нужно задать общую структуру
// или обязательные для реализации методы в производных классах.
//
//open класс — это обычный класс, от которого можно наследоваться. В отличие от abstract, из open класса можно
// создавать объекты напрямую. Он полезен, когда нужно создать базовый класс с реализацией методов, который можно
// расширить, но не обязательно реализовать в дочерних классах.
//
//Отличие:
//
...
//Когда применять:
//
//abstract — когда нужен шаблон для будущих классов и строгое наследование с обязательной реализацией методов.
//open — для создания класса, от которого можно наследовать, добавляя новые функции, но который сам по себе является
...
fun main() {
    var car = Car(400, 5, "12354","petrol","sedan")
    car.turnOnLights()
    car.stopMovement()
}
